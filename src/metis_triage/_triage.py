...
def load_config(path=None):
    """Load a complete JSON configuration or the defaults bundled in the package."""
    source = Path(path) if path is not None else files("metis_triage").joinpath("default_config.json")
...
def classify(issue, repository, available_labels, config, *, api_key=None, model=None):
    questions = {
        "category": {
            "type": "choice",
            "instructions": (
    ...
    response = request_json(
        ...
        api_key if api_key is not None else required_env("TYPESAFE_API_KEY"),
        method="POST",
        payload={
